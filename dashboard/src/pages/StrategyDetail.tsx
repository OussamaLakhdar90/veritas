...
const SECTIONS: Array<{ key: string; label: string }> = [
  ...
  { key: 'scope', label: 'Scope & objectives' },
  { key: 'riskRegister', label: 'Risk register' },
  { key: 'testApproach', label: 'Test approach' },
...
function SectionView({ k, value }: { k: string; value: unknown }) {
  ...
  if (k === 'scope' && value && typeof value === 'object') {
    return (
      <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
        <div><p className="mb-1 font-semibold text-ink-900">Objectives</p><ul className="list-disc space-y-0.5 pl-5 text-ink-700">{(d.objectives || []).map((x, i) => <li key={i}>{x}</li>)}</ul></div>
        <div>
          <p className="mb-1 font-semibold text-ink-900">In scope</p><ul className="list-disc space-y-0.5 pl-5 text-ink-700">{(d.inScope || []).map((x, i) => <li key={i}>{x}</li>)}</ul>
          {d.outOfScope?.length ? <><p className="mb-1 mt-3 font-semibold text-ink-900">Out of scope</p><ul className="list-disc space-y-0.5 pl-5 text-ink-700">{d.outOfScope.map((x, i) => <li key={i}>{x}</li>)}</ul></> : null}
        </div>
      </div>
    );
  }
  if (k === 'riskRegister' && Array.isArray(value)) {
    ...
    return (
      <Table head={<><Th>ID</Th><Th>Risk</Th><Th>Level</Th><Th>Mitigation</Th></>}>
        {(risks || []).map((r, i) => (
          <Row key={r.id ?? i}>
  ...
  if (k === 'testApproach' && value && typeof value === 'object') {
    return (
      <div className="text-sm">
        <p className="mb-2 text-ink-900"><span className="font-semibold">Levels:</span> {(d.levels || []).join(', ') || '—'} · <span className="font-semibold">Types:</span> {(d.types || []).join(', ') || '—'}</p>
  ...
  if (k === 'selfReview' && value && typeof value === 'object') {
    ...
    return (
      <div className="text-sm">
        {sr?.confidence != null && <p className="mb-2 text-ink-900">Confidence: <span className="font-semibold">{Math.round(sr.confidence)}%</span></p>}
        ...
        {sr?.blindSpots?.length ? <><p className="mb-1 mt-2 font-semibold text-ink-900">Blind spots</p><ul className="list-disc space-y-0.5 pl-5 text-warning">{sr.blindSpots.map((b, i) => <li key={i}>{b}</li>)}</ul></> : null}
      </div>
    );
  }
...
function StrategySection({ id, sectionKey, label, value, approved }: {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { invalidate(); setEditing(false); toast.push('success', `Saved "${label}" — a new version was created.`); },
    onError: (e: Error) => toast.push('error', e.message),
  });
  const regen = useMutation({
    mutationFn: () => api.regenerateStrategySection(id, sectionKey, guidance.trim() || undefined),
    onSuccess: () => { invalidate(); setGuidance(''); toast.push('success', `Regenerated "${label}".`); },
    onError: (e: Error) => toast.push('error', e.message),
  });
      ...
      <CardBody className="space-y-4">
        ...
        {editing && (
          <div className="space-y-3 border-t border-border pt-4">
            <Field label="Section content (JSON)" hint="Edit and save to store a new immutable version, or regenerate with the assistant.">
            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <div className="flex-1 min-w-[200px]">
                <Field label="Regenerate guidance (optional)">
                  <Input placeholder="e.g. emphasise security risks" value={guidance} onChange={(e) => setGuidance(e.target.value)} />
                </Field>
              </div>
              {notice}
    ...
    <Card className={`mb-5 border-l-4 ${ok ? 'border-l-success' : 'border-l-warning'}`}>
      <CardBody className="space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm text-ink-900">Quality scorecard · confidence {Math.round(sc.confidence)}%</span>
          <span className="text-[13px] text-muted">{sc.featuresCovered} feature(s) covered</span>
          {sc.droppedSections > 0 && <span className="text-[13px] font-medium text-warning">{sc.droppedSections} section(s) dropped</span>}
        </div>
        {sc.checks?.length ? (
...
export function StrategyDetail() {
  ...
  const approve = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.push('success', 'Strategy approved — release plans will now pin to this version.');
    },
    onError: (e: Error) => toast.push('error', e.message),
  });
  ...
  if (q.isError || !q.data) return <ErrorState message={`Could not load strategy: ${(q.error as Error)?.message ?? 'unknown error'}`} />;
  ...
  return (
    <div>
      <PageHeader
        title={`${s.serviceName ?? 'Strategy'} — Test Strategy`}
        subtitle={`v${(s as { version?: number }).version ?? 1} · ${s.status ?? 'DRAFT'}${s.confidence != null ? ` · ${Math.round(s.confidence)}% confidence` : ''}`}
        actions={
          <div className="flex items-center gap-2">
            ...
            {s.source === 'multi-source' && (
              <a href={api.strategyWhyDocUrl(s.id)} target="_blank" rel="noreferrer"
                className="inline-flex items-center gap-1 rounded-md px-3 py-2 text-[13px] font-medium text-ink-700 ring-1 ring-border hover:bg-ink-50">
                <ScrollText className="h-4 w-4" /> Evidence <ExternalLink className="h-3 w-3" />
              </a>
            )}
            {!approved && (
            ...
            {approved && <Badge className={TONE.ok}>Approved</Badge>}
          </div>
        } />
      ...
      {SECTIONS.map(({ key, label }) => (
        <StrategySection key={key} id={s.id} sectionKey={key} label={label} value={deliverable[key]} approved={approved} />
      ))}

      <Card className="mb-5">
        <CardHeader title={<span className="inline-flex items-center gap-2"><History className="h-4 w-4" /> Version history</span>}
          subtitle="Every edit or regenerate stores a new immutable version." />
        <CardBody className="p-0">
          {versions.length === 0 ? (
            <p className="px-5 py-4 text-sm text-muted">Only this version exists.</p>
          ) : (
            <Table head={<><Th>Version</Th><Th>Status</Th><Th>Revised by</Th><Th>Created</Th><Th /></>}>
              {versions.map((v) => {
                ...
                return (
                  <Row key={v.id}>
                    ...
                    <Td className="text-right">
                      {v.id === s.id ? <span className="text-[13px] text-muted">current</span> :
                        <Link to={`/test-strategy/${v.id}`} className="text-[13px] font-medium text-gold hover:underline">Open</Link>}
                    </Td>
                  </Row>
                );
              })}

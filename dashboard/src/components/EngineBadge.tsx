...
export function EngineBadge() {
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      {simulated && (
        <Tooltip label={t('engine.mockTooltip')}>
          <span className="inline-flex items-center gap-1 rounded-full bg-warning/10 px-2.5 py-1 text-2xs font-medium text-warning ring-1 ring-warning/30">
            <FlaskConical className="h-3 w-3" /> {t('engine.simulatedData')}
          </span>
        </Tooltip>
      )}
      {seeded && (
        <Tooltip label={t('engine.seededTooltip')}>
          <span className="inline-flex items-center gap-1 rounded-full bg-gold/10 px-2.5 py-1 text-2xs font-medium text-gold ring-1 ring-gold/30">

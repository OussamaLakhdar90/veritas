...
1. **`FeatureSeeder` ($0)** — bucket units by shared `hints` (Jira components/labels, Confluence section slugs, code path-nouns). Degrades gracefully to title/path-noun overlap when labels/components are absent. Rough buckets that also shrink what the LLM sees and bound the tagger call. **Over-merge guard (implemented):** because prose hints are free-text words, linking on a *single* shared generic noun ("user", "policy") would transitively collapse unrelated features into one blob; so two units are unioned only when they share **≥2 stemmed hints** (the code adapter adds the controller class as a high-precision hint so same-controller endpoints still cluster). The seed is deliberately **conservative** — under-clustering is the recoverable direction; the LLM tagger (§3.2) MERGES (intent↔endpoint cross-source, semantic synonyms), it never splits, so the seed must not over-merge.
...
- (b) a `$0` **cross-feature lexical near-match guard** (implemented): an intent-only feature and a code-only feature that share **≥ `MIN_SHARED_HINTS` (2)** stemmed hints — a lexical near-miss the conservative seed couldn't bridge via any single unit pair — are flagged "possible mis-cluster" into the §6 preview. (This catches lexical near-misses, not true **synonyms** — login vs authentication share zero stems; bridging synonyms is the LLM tagger's job, not this deterministic guard. The ≥2 bar mirrors the seeder's so the guard doesn't flag the single-shared-noun overlaps the seed was right to keep apart.)
...
1. **Evidence model** — new `evidence` package: `EvidenceUnit` (with content-hash ids, lifecycle/priority/links), `SourceKind` (incl. `POLICY`), `UnitType` (incl. `DTO_CONSTRAINT`, `GLOBAL_CAVEAT`, `STANDARD`), `EvidenceExtractor` (three adapters + §1.3 fetch contract + §2.1 `Redactor`); **`Endpoint` gains a controller class name** + the `CODE:<Class>#<HTTP> <path>` id; the `DTO_CONSTRAINT` schema traversal of `ApiModel.schemas()`; `GLOBAL_CAVEAT` emission from `ApiModel.blindSpots()`. **Jira widening is multi-file**, not a constant edit: extend `JiraIssue` (status/priority/components/labels/links/url), parse them in **both** edition clients (v3/ADF nested-name components vs v2/wiki), fix the two hardcoded `?fields=summary,description` lists, add a Cloud + Server round-trip fixture test. **Description edition split:** Cloud is ADF (handled by `AdfToMarkdown`), but **Server/DC is wiki markup** — the widening must add a wiki→markdown path, else the `JiraEvidenceAdapter` silently gets only the summary on the default Server/DC edition (so the Phase-3 `FeatureSeeder` isn't fed empty Jira bodies)._Phase 2 status: the `EvidenceExtractor` + the three adapters + `SourceSelection`/`ExtractionResult` are built (merged); the `Endpoint` declaring-class change is deferred — the Code adapter derives the class from the source file basename, which is unique by method+path._

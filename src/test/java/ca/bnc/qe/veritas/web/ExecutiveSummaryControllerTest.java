...
@Import(ca.bnc.qe.veritas.config.GateProperties.class)   // the gate POJO isn't part of the web slice
class ExecutiveSummaryControllerTest {
    ...
    private static Scan scan(String id, String service, int daysAgo) {
        Scan s = new Scan();
    ...
    void rollsUpVerdictsDispositionsAndBreakingTotal() throws Exception {
        // payments: an OLDER scan must be ignored — only s2 (the latest, all-additive drift → WARN) counts.
        ...
        mvc.perform(get("/api/v1/summary/executive"))
                ...
                .andExpect(jsonPath("$.perService[1].releaseSafe").value("WARN"))

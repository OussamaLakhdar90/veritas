...
| `analyze-test-conditions --name <svc>` (`--repo`/`--app-id` or `--jql`/`--confluence`) | ISTQB **test analysis**: identify + prioritize the test conditions ("what to test") from the strategy + basis; each traces a basis item + a risk and carries an automation candidacy (AUTOMATED → `implement-tests`, MANUAL → `create-test-cases`). **Code-first:** if no strategy exists yet, a DRAFT one is auto-generated from the same basis so the whole chain runs from just the source (the hard "APPROVED strategy" gate still applies to the outward release plan). The work product between the strategy and the cases. |
